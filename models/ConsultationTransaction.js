...
const consultationTransactionSchema = new mongoose.Schema({
  ...
  transactionId: {
    ...
    unique: true,
    required: true,
    description: 'Unique transaction ID for this consultation payment'
  },
  ...
  doctorName: {
    type: String,
    required: true,
    description: 'Doctor name at time of payment'
  },
  ...
  timestamps: true,
  collection: 'consultationtransactions'
});

// Add indexes for efficient querying
consultationTransactionSchema.index({ transactionId: 1 }, { unique: true });
consultationTransactionSchema.index({ patientId: 1 });
consultationTransactionSchema.index({ doctorId: 1 });
consultationTransactionSchema.index({ centerId: 1 });
consultationTransactionSchema.index({ processedBy: 1 });
consultationTransactionSchema.index({ status: 1 });
consultationTransactionSchema.index({ createdAt: -1 });
consultationTransactionSchema.index({ processedAt: -1 });
consultationTransactionSchema.index({ consultationType: 1 });
consultationTransactionSchema.index({ centerId: 1, processedAt: -1 });

// Pre-save middleware to update timestamps and transaction ID
consultationTransactionSchema.pre('save', function (next) {
  if (this.isNew) {
    // Generate transaction ID if not provided
    if (!this.transactionId) {
      const timestamp = Date.now();
      const randomPart = Math.random().toString(36).substr(2, 5);
      this.transactionId = `CONSULT-${timestamp}-${randomPart.toUpperCase()}`;
    }
...
consultationTransactionSchema.methods.updateStatus = function(newStatus, changedBy, reason, notes) {
...
consultationTransactionSchema.methods.addRefund = function(refundData) {
...
consultationTransactionSchema.virtual('remainingRefundAmount').get(function() {
  const totalRefunded = this.refunds.reduce((sum, refund) => sum + refund.amount, 0);
  return Math.max(0, this.amount - totalRefunded);
});

// Virtual for total refunded amount
consultationTransactionSchema.virtual('totalRefundedAmount').get(function() {
  return this.refunds.reduce((sum, refund) => sum + refund.amount, 0);
});

// JSON transformation options
consultationTransactionSchema.set('toJSON', { virtuals: true });
consultationTransactionSchema.set('toObject', { virtuals: true });

const ConsultationTransaction = mongoose.model('ConsultationTransaction', consultationTransactionSchema);
